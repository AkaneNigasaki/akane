...
export default function Home() {
  ...
  return <div id="top" className="min-h-screen bg-background text-foreground font-sans">
    ...
    <main className="mx-auto w-full max-w-[1440px] px-4 sm:px-8 lg:px-12">
      ...
      {playerData && !isLoadingPlayer && <div className="player-results border-x border-b border-border px-5 py-8 sm:px-12 lg:px-20"><section aria-labelledby="identity-heading" className="result-section"><div className="section-kicker">Profil détecté</div><h3 id="identity-heading" className="sr-only">Identité et état du compte</h3><PlayerCard player={playerData} /></section><section aria-labelledby="performance-heading" className="result-section"><div className="section-heading"><div><div className="section-kicker">Performance</div><h3 id="performance-heading">Lecture des performances</h3></div><p>Une vue synthétique des habitudes de jeu et des indicateurs de progression.</p></div><PlayerSpiderChart player={playerData} /><PlayerRankPrime player={playerData} /><PlayerStats stats={playerData.stats} clan={playerData.clan} pet={playerData.pet} social={playerData.social} /></section><section aria-labelledby="context-heading" className="result-section"><div className="section-heading"><div><div className="section-kicker">Contexte</div><h3 id="context-heading">Comprendre les données</h3></div><p>Les explications et la réponse brute restent séparées pour une lecture plus fiable.</p></div><PlayerDeepExplanations player={playerData} /><RawDataViewer player={playerData} /></section><section id="developer" aria-labelledby="developer-heading" className="result-section"><div className="section-heading"><div><div className="section-kicker">Développeurs</div><h3 id="developer-heading">Intégrer FF Inspector</h3></div><p>Réutilisez les informations vérifiées dans vos propres outils.</p></div><DeveloperTools playerData={playerData} currentUid={currentUid} /></section></div>}
    </main>
